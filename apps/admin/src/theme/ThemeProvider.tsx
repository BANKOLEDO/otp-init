...
export function AppThemeProvider({ children }: { children: ReactNode }) {
  const [mode, setMode] = useState<Mode>(() => (localStorage.getItem('saas-theme') as Mode) || 'light');
  const toggle = () => setMode((p) => { const n = p === 'light' ? 'dark' : 'light'; localStorage.setItem('saas-theme', n); return n; });
